...
> This README covers the default **end-to-end-encrypted** relay (untrusted
> server). For the **trusted** managed-server protocol — GitHub login,
> server-decided visibility, and attributed triage — see
> [`server-managed/MANAGED.md`](../server-managed/MANAGED.md).
...
- **`TRUST_PROXY=1`** — make the same-origin gate honour `X-Forwarded-Host`
  ...
  Neon-specific — the gate (`server-e2e/origin.ts`) defaults on for a loopback
...
The WS fan-out is still an in-memory subscriber map (`server-e2e/hub.ts`),
but Neon mode wires a Postgres LISTEN/NOTIFY bus
(`server-e2e/pubsub.ts`) alongside it so live broadcasts reach peers on
...
```sh
cp server-e2e/config.example.json server-e2e/config.json
```
...
`server-e2e/objstore/sign.ts` (`canonicalObjstorePut` / `…Delete` / `…Fetch` /
...
(`server-e2e/objstore/rest-mint.ts`): authed by an Ed25519 signature in the JSON
...
dedup (`server-e2e/objstore/fetch-mint-guard.ts`), so a retry must re-sign with a
...
(`server-e2e/objstore/store.ts`), while Neon relies on native Postgres types
(`BIGINT`, etc.) and carries the same `CHECK`s
(`server-e2e/objstore/store-neon.ts`).
...
(`server-e2e/objstore/blob.ts`). Bytes live outside the metadata store to keep
...
UNIQUE-violation recovery branch (`catch` in `server-e2e/db.ts`) re-routes a
