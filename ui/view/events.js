import { KANBAN_DETAIL_FULLSCREEN_KEY, SEVERITY_MODE_KEY, VIEW_MODE_KEY, isEncryptionEnabled, patchEntry, readBundle, saveRepoUrlFor, saveTriage, setReportIgnored, setUpstream, state, subscribeToBundleFindingIndex, subscribeToBundleHashIndex, upstreamOf } from '#client/index.js'
import { downloadBlob, report } from './dom.js'
import { commonPrefix, configureRevalidation, handoffBlock, lineRange } from './format.js'
import { activeTabFor, canApplyFixToGroup, findGroupById, findingPackage, findingRepo, findingReport, fixApplies, getMergedGroups, groupState, groupWithPassRows, setTabFix, setTabTriage, syncGroupTriage, tabFix, tabKey, triageActionPlan, triageAppScope, triageScope } from './group.js'
...
report.addEventListener('click', (e) => {
  ...
  //
  // Set-or-clear is `triageActionPlan`'s call, made ONCE for the whole
  // scope before the loop: the same answer has to reach every tab, or
  // a re-click flips the tabs holding the state off and the rest on
  // instead of switching the group off.
  ...
  if (triageActionBtn) {
    ...
    // `clearing` covers Restore and a re-click of the state the scope
...
report.addEventListener('row-select', (e) => {
  ...
  if (!gid) return
  const opening = state.tableSelectedGid !== gid
  state.tableSelectedGid = opening ? gid : null
  if (opening) syncOpenedGroupTriage(gid)
  renderPreservingTableScroll()
})
...
// Write one triage target onto every tab in `targets`. The two axes
...
// `<revalidate-filter>` — the toolbar dropdown for the revalidation
// pass's outcome. Same shape as the repo filter above: the component
// owns the select, the state write and re-render land here.
report.addEventListener('revalidate-change', (e) => {
