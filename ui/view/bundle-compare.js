...
class BundleCompare extends LitElement {
  ...
  _diffFor() {
    ...
    if (this._diffKey !== key || !this._diff) {
      ...
      this._diff = computeBundleDiff(baseSources, otherSources, bundlePkgOf)
      this._diffKey = key
    }
    return this._diff
  }
  ...
  _renderPicker(others) {
    ...
    return html`<div class="bundle-compare-picker">
      ...
      <label class="bundle-compare-select-wrap">
        ...
        <select class="bundle-compare-select" @change=${(e) => this._onPick(e)} aria-label="Bundle to compare with">
          <option value="" ?selected=${!this._targetIntegrity}>Choose a bundle…</option>
          ${others.map((o) => html`<option value=${o.integrity} ?selected=${o.integrity === this._targetIntegrity}>${o.label}</option>`)}
        </select>
      </label>
    </div>`
  }
