...
// 14px to match the chrome's other icon buttons. The default JSON
// bucket renders the plain outline. Source-marked groups switch to
// a filled "sticker" treatment: the sheet body fills with the
// upstream's brand color (Claude salmon, OpenAI white, Vercel
// black), a translucent black triangle in the corner reads as a
// folded-over flap, and the official mark sits centered in the
// foreground color. Bg / fg fills are themed via the
// `.brand-claude` / `.brand-codex` / `.brand-vercel` classes on the
// SVG root in sidebar.css; path coordinates have been baked through
// svgo so the brand glyphs land at their final positions without
// runtime transforms. Re-baked into each row's HTML rather than
// referenced by id so a single `innerHTML` write paints the whole
// list.
