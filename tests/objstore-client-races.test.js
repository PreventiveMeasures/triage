...
// v1.objstore extension. Boots a real server (server-e2e/index.ts in a
...
describe('objstore client/server races', { concurrency: true }, () => {
  ...
  it('onDeleted fires for OWN deletes (broadcast symmetry with onPut)', async () => {
    // PUT broadcasts in `server-e2e/objstore/rest.ts` are emitted with
    // `except: null` (every subscriber including the originator
    // sees them); DELETE broadcasts in `server-e2e/objstore/handlers.ts`
