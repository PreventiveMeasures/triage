// Test-only: exercise `server-e2e/db-neon.ts`'s Neon code path against an
...
//
// `openNeonDb` dynamically imports `server-e2e/neon-driver.ts` — the single
...
import { openNeonDb } from '../server-e2e/db-neon.ts'
import { openNeonObjstore } from '../server-e2e/objstore/store-neon.ts'
import { openFsBlobBackend } from '../server-e2e/objstore/blob-fs.ts'
...
mock.module('../server-e2e/neon-driver.ts', {
