...
import { abortPut, beginPut, commitPut, deleteObject, getLive, openObjstore } from '../server-e2e/objstore/store.ts'
import { reapOrphans } from '../server-e2e/objstore/reaper.ts'
import { handleRest } from '../server-e2e/objstore/rest.ts'
import { mintGetToken, newTokenSecret } from '../server-e2e/objstore/tokens.ts'
...
// builds in server-e2e/index.ts (modulo SQLite ↔ Neon on the DB plane).
let counter = 0
async function freshVercelHandle() {
  ...
  // (server-e2e/index.ts builds the Neon handle from openNeonObjstore,
