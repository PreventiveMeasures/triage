...
test('unattributed deletion checks current admin role and version and audits the acting user', async t => {
  const db = openSqliteManagedDb(':memory:')
  t.after(() => db.close())
  const admin = await db.upsertUser(identity(1, 'admin'), 10)
  const manager = await db.upsertUser(identity(2, 'manager'), 10)
  await db.setUserRole(admin, 'admin')
  await db.setUserRole(manager, 'manage')
  const anonymous = await db.createComment({ findingId: 'f', body: 'Imported body', authorId: null, authorLogin: 'former-user' }, 20)
  const authored = await db.createComment({ findingId: 'f', body: 'Owned body', authorId: manager, authorLogin: 'manager' }, 21)
  assert.equal(await db.deleteComment(anonymous.id, manager, 'manager', 1, 'r', 30), 'forbidden')
  assert.equal(await db.deleteComment(authored.id, admin, 'admin', 1, 'r', 30), 'forbidden')
  assert.equal(await db.editComment(anonymous.id, admin, 'admin', 'Claim text', 1, 'r', 30), 'forbidden')
  assert.equal(await db.deleteComment(anonymous.id, admin, 'admin', 2, 'r', 30), 'conflict')
  await db.setUserRole(admin, 'manage')
  assert.equal(await db.deleteComment(anonymous.id, admin, 'admin', 1, 'r', 30), 'forbidden')
  assert.deepEqual(await db.getComment(anonymous.id), anonymous)
  await db.setUserRole(admin, 'admin')
  assert.equal(await db.deleteComment(anonymous.id, admin, 'admin', 1, 'r', 40), 'deleted')
  assert.deepEqual(await db.listComments(['f']), [authored])
  const history = await db.listActivity({ page: 1, limit: 100, kind: 'triage', query: '', contexts: null })
  assert.equal(history.total, 3, 'failed deletions do not add audit events')
  assert.equal(history.history[0].action, 'deleted a comment')
  assert.equal(history.history[0].actor, 'admin')
  assert.doesNotMatch(JSON.stringify(history), /Imported body|Owned body/u)
  assert.equal((await db.listUsers()).find(user => user.id === admin).lastActivityAt, 40)
})
...
test('comment DELETE requires current ownership, access, CSRF and version', async t => {
  ...
  assert.equal((await request('DELETE', `${path}/${anonymous.id}`, 'bob', { version: 1 })).status, 403)
...
test('admins can DELETE unattributed comments with CSRF, finding access and a current version', async t => {
  const { db, sessions, request } = await fixture(t)
  const comment = await db.createComment({ findingId: 'own', body: 'Imported note', authorId: null, authorLogin: null, createdAt: null }, 20)
  const path = `/api/reports/r/comments/${comment.id}`
  assert.equal((await request('DELETE', path, null, { version: 1 })).status, 401)
  assert.equal((await request('DELETE', path, 'admin', { version: 1 }, false)).status, 403)
  for (const user of ['alice', 'bob']) assert.equal((await request('DELETE', path, user, { version: 1 })).status, 403)
  for (const user of ['reader', 'outside']) assert.equal((await request('DELETE', path, user, { version: 1 })).status, 404)
  assert.equal((await request('DELETE', path, 'admin', {})).status, 400)
  assert.equal((await request('DELETE', path, 'admin', { version: 2 })).status, 409)
  assert.equal((await request('PATCH', path, 'admin', { body: 'Claim note', version: 1 })).status, 403)
  assert.deepEqual(await db.getComment(comment.id), comment)
  const unrelated = await db.createComment({ findingId: 'not-in-report', body: 'Unrelated note', authorId: null, authorLogin: null }, 20)
  assert.equal((await request('DELETE', `/api/reports/r/comments/${unrelated.id}`, 'admin', { version: 1 })).status, 404)
  assert.equal((await request('DELETE', path, 'admin', { version: 1 })).status, 204)
  assert.equal((await request('DELETE', path, 'admin', { version: 1 })).status, 404)
  assert.equal(await db.getComment(comment.id), null)
  const history = await request('GET', '/api/admin/history?kind=triage', 'admin')
  assert.equal(history.history[0].action, 'deleted a comment')
  assert.equal(history.history[0].actor, 'admin')
  const former = await db.createComment({ findingId: 'own', body: 'Former user note', authorId: null, authorLogin: 'former-user' }, 20)
  const formerPath = `/api/reports/r/comments/${former.id}`
  await db.setUserRole(sessions.admin.userId, 'none')
  assert.equal((await request('DELETE', formerPath, 'admin', { version: 1 })).status, 403)
  assert.deepEqual(await db.getComment(former.id), former)
  await db.setUserRole(sessions.admin.userId, 'admin')
  assert.equal((await request('DELETE', formerPath, 'admin', { version: 1 })).status, 204)
})
