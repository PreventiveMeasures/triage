...
test('managed preview triage persists in memory and stays scoped to the requested report', async (t) => {
  ...
  await t.test('preview history filters repositories and users before pagination', async () => {
    ...
    assert.equal(first.filters.users.length, 3)
    const user = await get('repo=example%2Fmanaged-fixtures&actor=user:fixture-alex&limit=1&page=2')
    assert.equal(user.total, 2)
    assert.equal(user.page, 2)
    assert.equal(user.history[0].actor, 'alex-security')
    assert.equal((await get('actor=user:fixture-alex&repo=example%2Fworker-service')).total, 0)
    assert.equal((await get('actor=unknown')).total, 0)
  })
