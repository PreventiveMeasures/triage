...
function signPut(sk, fields, connectionNonce) {
  // Field order mirrors server-e2e/objstore/sign.ts:canonicalObjstorePut —
...
function signDelete(sk, fields, connectionNonce) {
  ...
  // (server-e2e/objstore/sign.ts:canonicalObjstoreDelete).
  return sign(sk, encodeUtf8([
...
// matching server-e2e/objstore/store.ts:randomId + isValidIncarnation).
...
describe('v1.objstore server (REST-primary)', { concurrency: true }, () => {
  ...
  it('put-begin with non-safe-int prevVersion → silent drop (handler gate matches sig-verifier)', async () => {
    ...
    // `server-e2e/objstore/handlers.ts:76`. (NaN / Infinity aren't
  ...
  // byte counter at server-e2e/objstore/rest.ts has a defensive
  ...
  it('101st NEW resource per workspace → objstore-put-error { reason: workspace-full } (H1 cap)', async () => {
    // Audit round-9 H1: server-e2e/objstore/store.ts caps live rows per
