...
test('static serving includes provider SVGs without opening arbitrary subdirectories', async () => {
  ...
  try {
    ...
    assert.equal(handle({ method: 'GET', url: '/provider-icons/openai.svg', headers: {} }, response), true)
    ...
    for (const url of ['/provider-icons/private.txt', '/provider-icons/link.svg', '/private/key.svg', '/provider-icons/../private/key.svg', '/provider-icons/%2e%2e/private/key.svg']) {
