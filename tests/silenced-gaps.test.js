...
//
// The rule is a containment check against stderr, which is subtle
// enough to be worth pinning from both ends: a gap the transcript
// already shows must not be repeated as a hint, and a gap silenced by
// a redirect, a pipe, a gate or a subshell must not be lost. Both are
// checked against real `@preventive/terminal` runs as well as hand-
// built results, because the relationship between the two channels is
...
describe('silencedGaps — the containment rule', () => {
  it('reports nothing when there are no gaps', () => {
    assert.deepEqual(silencedGaps({ stderr: '', unsupported: [] }), [])
    assert.deepEqual(silencedGaps({ stderr: 'cat: nope: No such file\n', unsupported: [] }), [])
  })

  it('leaves out a gap whose message reached stderr', () => {
    const gap = { message: 'ls: unknown option: --bogus' }
    assert.deepEqual(silencedGaps({ stderr: 'ls: unknown option: --bogus\n', unsupported: [gap] }), [])
  })

  it('reports a gap that stderr never carried', () => {
    const gap = { message: 'ls: unknown option: --bogus' }
    assert.deepEqual(silencedGaps({ stderr: '', unsupported: [gap] }), ['ls: unknown option: --bogus'])
  })
  ...
  it('counts a shell-level gap as visible despite the `error: ` prefix', () => {
    ...
    assert.deepEqual(silencedGaps({ stderr, unsupported: [gap] }), [])
  })
  ...
  it('splits a mixed run, keeping only the silenced half', () => {
    ...
    assert.deepEqual(silencedGaps({ stderr, unsupported: [seen, hidden] }), ['cat: unknown option: --nope'])
  })
})
