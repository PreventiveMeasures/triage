...
//
// Lives OUTSIDE `client/sync.js` deliberately. The sync chunk's entry
// must not import non-sync `client/*` files, else the chunk re-bundles
// e.g. `client/triage.js` and the `triageChangeNotifier` slot saveTriage
// flips ends up forked across the main `view.js` bundle and the lazy
// chunk — `saveTriage` (main bundle) writes the main slot, the chunk's
// install wires the chunk's slot, and notifications silently no-op.
//
// `applyDefaultSyncHost(syncMod)` is the install entry: pass the loaded
// sync module namespace (via `await import('./sync.js')` in the UI
// loader or a static import in tests). It installs the host and wires
// the `setTriageChangeNotifier` slot to the module's `triageSync.notify`.
...
export function applyDefaultSyncHost(syncMod) {
  ...
  // Wire triage's tail-of-save notifier so `saveTriage` fans the change
  // out to peers via `triageSync.notify`. The slot lives on THIS bundle's
  // `client/triage.js` instance — the one `saveTriage` (called from UI
  // code in the main bundle) writes through. The
  // `() => syncMod.triageSync.notify()` lookup is late-bound so tests
  // that stub `triageSync.notify = ...` still see their stub invoked.
