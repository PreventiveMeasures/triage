...
export function createObjstoreClient(deps: ObjstoreClientDeps): ObjstoreClient {
  ...
  function seedInventory(state: SessionState, resources: ObjectMeta[]): void {
    ...
    for (const m of resources) {
      // Overlapping subscription queries can finish out of order. Keep
      // newer metadata already received for this incarnation, including
      // through a broadcast or an earlier row in this snapshot.
      const known = state.inventory.get(m.resourceTag) ?? previous.get(m.resourceTag)
      const current = known?.incarnation === m.incarnation && known.version >= m.version
        ? known
        : { version: m.version, incarnation: m.incarnation, contentLength: m.contentLength }
      state.inventory.set(m.resourceTag, current)
      noteVersion(state, m.resourceTag, current.incarnation, current.version)
    }
    state.resolveListed()
  ...
  function onTransportMessage(msg: WireMessage): void {
    ...
    if (msg.type === 'objstore-deleted' && typeof msg.workspaceTag === 'string' && typeof msg.resourceTag === 'string' && typeof msg['version'] === 'number') {
      ...
      // A tombstone may arrive after a snapshot/put of a new incarnation,
      // including after waiting in the subscription's broadcast buffer.
      // Missing incarnation retains compatibility with older relays.
      const incarnation = msg['incarnation']
      if (incarnation !== undefined) {
        if (typeof incarnation !== 'string' || incarnation.length === 0) return
        const current = state.inventory.get(tag) ?? state.seenVersions.get(tag)
        if (current && (incarnation !== current.incarnation || current.version > version)) return
        // With no current row, still deliver an echo after our own delete
        // acknowledgment has cleared the inventory and rollback watermark.
      }
